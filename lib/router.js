...
Router.prototype._parseRoutes = function() {
  var map = this._makeMap();
  this.routes(map);
};

Router.prototype._makeMap = function(prefix) {
  prefix = prefix || '';
  ...
  this.verbs.forEach(function (verb) {
    map[verb] = function (route, actionString, options) {
      ...
      self._registerRoute(verb, prefix + route, options);
    };
  });
  ...
  map.controller = function (controllerName, options) {
    ...
    actions.forEach(function (action) {
      ...
      self._registerRoute(options.method, prefix + options.prefix + action, routeOptions);
    });
  };

  // Add a namespace helper that calls a callback with a map object
  // that automatically prefixes all routes
  map.namespace = function (namespaceName, callback) {
    var map = self._makeMap(prefix + namespaceName);
    callback(map);
  }
...
Router.prototype._registerRoute = function(method, route, options) {
  ...
  this.app[method](route, this.options.routeWrapper(function (req, res, next) {
    ...
    return controller.dispatch(req, res, next, options);
  }));
};
